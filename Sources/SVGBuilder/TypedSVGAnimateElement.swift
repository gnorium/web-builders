...
public struct TypedSVGAnimateElement<Value>: SVGElementRenderable, Sendable {
    ...
    public func addingAttribute(_ key: String, _ value: String) -> TypedSVGAnimateElement<Value> {
        var newAttributes = attributes
        newAttributes.removeAll { $0.0 == key }
        newAttributes.append((key, value))
        return TypedSVGAnimateElement(attribute: attribute, attributes: newAttributes)
    }
}
...
extension TypedSVGAnimateElement where Value == Length {
    ...
    public func values(_ items: Length...) -> Self { addingAttribute("values", stringJoin(items.map { $0.value }, separator: ";")) }
}
...
extension TypedSVGAnimateElement where Value == Percentage {
    ...
    public func values(_ items: Percentage...) -> Self { addingAttribute("values", stringJoin(items.map { $0.value }, separator: ";")) }
}
...
extension TypedSVGAnimateElement where Value == Double {
    ...
    public func values(_ items: Double...) -> Self { addingAttribute("values", stringJoin(items.map { doubleToString($0) }, separator: ";")) }
}

extension TypedSVGAnimateElement where Value == [(Double, Double)] {
    public func from(_ coordinates: (Double, Double)...) -> Self {
        let str = stringJoin(coordinates.map { stringConcat(doubleToString($0.0), ",", doubleToString($0.1)) }, separator: " ")
    ...
    public func values(_ steps: [(Double, Double)]...) -> Self {
        let strings = steps.map { coords in
            stringJoin(coords.map { stringConcat(doubleToString($0.0), ",", doubleToString($0.1)) }, separator: " ")
        }
...
extension TypedSVGAnimateElement {
    ...
    public func repeatCount(_ count: SVGAnimate.RepeatCount) -> Self { addingAttribute("repeatCount", count.value) }
    ...
    public func attributeType(_ type: SVGAnimate.AttributeType) -> Self { addingAttribute("attributeType", type.rawValue) }
    ...
    public func calcMode(_ mode: SVGAnimate.CalcMode) -> Self { addingAttribute("calcMode", mode.rawValue) }
