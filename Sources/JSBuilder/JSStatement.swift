...
public indirect enum JSStatement: JSContent {
  ...
  public func build(indent: Int = 0) -> String {
    ...
    case .constDestructure(let names, let expr):
      var nameStrs: [String] = []
      for name in names {
        if case .identifier(let n) = name.expression {
          nameStrs.append(n)
        } else {
          nameStrs.append(name.expression.build())
        }
      }
...
public func function(
  _ name: String, _ params: [JSIdentifier], @JSBuilder _ content: () -> [JSStatement]
) -> JSStatement {
...
public func `if`(
  _ condition: JSExpression, @JSBuilder thenBlock: () -> [JSStatement],
  @JSBuilder elseBlock: () -> [JSStatement]
) -> JSStatement {
  return .if(condition, thenBlock(), elseBlock())
}

/// Create if statement without else block
public func `if`(_ condition: JSExpression, @JSBuilder thenBlock: () -> [JSStatement])
  -> JSStatement
{
...
public struct JSTryCatch {
  ...
  public func `catch`(_ catchVar: JSIdentifier, @JSBuilder catchBody: () -> [JSStatement])
    -> JSStatement
  {
    if case .identifier(let varName) = catchVar.expression {
      return .tryBlock(tryStatements, varName, catchBody())
    }
    return .tryBlock(tryStatements, catchVar.expression.build(), catchBody())
  }
}
...
public func `try`(
  @JSBuilder body: () -> [JSStatement], catch catchVar: String,
  @JSBuilder catchBody: () -> [JSStatement]
) -> JSStatement {
...
public func `for`(
  _ initializer: String, _ condition: JSExpression, _ increment: String,
