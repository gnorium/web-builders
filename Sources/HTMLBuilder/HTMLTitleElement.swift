...
public struct HTMLTitleElement: HTMLElementProtocol, Sendable, CustomStringConvertible {
	...
	public func render(indent: Int = 0) -> String {
		...
		let openElement = "<title\(attributeString)>"
		let closeElement = "</title>"
	...
	public func callAsFunction(@HTMLBuilder content: () -> [any HTMLProtocol]) -> HTMLTitleElement {
		HTMLTitleElement(attributes: attributes, children: content())
	}
	...
	public func addingAttribute(_ key: String, _ value: String) -> HTMLTitleElement {
		...
		return HTMLTitleElement(attributes: newAttributes, children: children)
	}

	public func style(prefix: Bool = true, @CSSBuilder _ content: () -> [any CSSProtocol]) -> HTMLTitleElement {
...
public func title(@HTMLBuilder content: () -> [any HTMLProtocol] = { [] }) -> HTMLTitleElement {
	HTMLTitleElement(content: content)
}

public func title(_ text: String) -> HTMLTitleElement {
	HTMLTitleElement(content: {
