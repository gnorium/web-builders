...
import WebTypes
...
#if SERVER
  extension CSS {
    public enum PseudoElement: String, Equatable {
      case before = "::before"
      case after = "::after"
  ...
  extension CSS {
    public enum PseudoElement: Equatable, RawRepresentable {
      case before
      case after
      case firstLine
      case firstLetter
...
public func pseudoElement(
  _ pseudoElement: CSS.PseudoElement, @CSSBuilder _ content: () -> CSSOM.CSSStyleDeclaration
) -> CSSOM.CSSStyleRule {
  CSSOM.CSSStyleRule(pseudoElement.rawValue, style: content())
}

public func pseudoElement(
  _ selector: String, _ pseudoElement: CSS.PseudoElement, @CSSBuilder _ content: () -> CSSOM.CSSStyleDeclaration
) -> CSSOM.CSSStyleRule {
  CSSOM.CSSStyleRule("\(selector)\(pseudoElement.rawValue)", style: content())
}

public func pseudoElement(_ selector: String, @CSSBuilder _ content: () -> CSSOM.CSSStyleDeclaration)
  -> CSSOM.CSSStyleRule
{
  CSSOM.CSSStyleRule(selector, style: content())
}

public func pseudoElement(_ selector: String, _ pseudoElement: CSS.PseudoElement) -> String {
  "\(selector)\(pseudoElement.rawValue)"
}

public func pseudoElement(_ pseudoElement: CSS.PseudoElement) -> String {
